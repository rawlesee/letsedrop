...
const API_BASE_URL = (import.meta as unknown as { env?: { VITE_API_BASE_URL?: string } }).env?.VITE_API_BASE_URL || DEFAULT_API_BASE_URL;
...
export default function App() {
  ...
  const saveToHistory = (item: Omit<HistoryItem, 'id' | 'timestamp'>) => {
    ...
    const updated = [newItem, ...history.filter((h) => h.url !== item.url).slice(0, 14)];
    setHistory(updated);
    try {
  ...
  const triggerBrowserDownload = (blob: Blob, filename: string, contentType: string) => {
  ...
  const handleAnalyze = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.message || 'Media tidak tersedia atau platform belum didukung.');
      }

      setAnalyzedUrl(cleanInput);

      if (result.type === 'image' || (Array.isArray(result.items) && result.items.length > 0)) {
        const items: MediaItem[] = result.items || [
          { index: 0, type: 'image', url: result.thumbnail || '', thumbnail: result.thumbnail || '', ext: 'jpg' },
        ];
        ...
        const formats: MediaFormat[] = [
          { id: 'best', label: 'Video MP4 (HD)', extension: 'mp4', quality: 'HD', type: 'video' },
          { id: 'audio', label: 'Audio MP3', extension: 'mp3', quality: 'High Audio', type: 'audio' },
        ];
        setMediaData({ ...result, type: 'video', formats });
        setSelectedFormatId('best');
      }
    } catch (err: unknown) {
      const error = err as Error;
      let humanMessage = error.message || 'Terjadi masalah saat memproses tautan media.';
      const lower = humanMessage.toLowerCase();
      if (lower.includes('private') || lower.includes('login')) {
        humanMessage = 'Media ini tidak dapat diakses karena bersifat privat atau membutuhkan login akun.';
      } else if (lower.includes('failed to fetch') || lower.includes('networkerror')) {
        humanMessage = 'Gagal menghubungi server Letsedrop. Periksa koneksi internet Anda lalu coba lagi.';
      }
  ...
  const handleDownloadVideo = async () => {
    ...
    try {
      setDownloadStepText('Mengunduh biner video...');
      const response = await fetch(`${API_BASE_URL}/api/download`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ url: urlToSend, formatId: payloadFormatId }),
      });

      if (!response.ok) {
        let errDesc = 'Gagal mengunduh file media dari server.';
        try {
      ...
      let finalFilename = isAudio ? 'Letsedrop_Audio.mp3' : 'Letsedrop_Video.mp4';
      ...
      triggerBrowserDownload(blob, finalFilename, contentType);
      ...
      setSuccessInfo({
        filename: finalFilename,
        platform: (mediaData.platform || 'VIDEO').toUpperCase(),
      });
    } catch (err: unknown) {
      const error = err as Error;
      setErrorMessage(error.message || 'Gagal memproses pengunduhan video.');
    } finally {
  ...
  const downloadSingleImage = async (item: MediaItem, customUrl?: string): Promise<void> => {
    const urlToSend = customUrl || analyzedUrl.trim();
    setDownloadingItemIndex(item.index);
    setErrorMessage(null);

    try {
      const response = await fetch(`${API_BASE_URL}/api/download-image`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          imageUrl: item.url,
      ...
      if (!response.ok) throw new Error('Foto tidak dapat diunduh saat ini.');

      let filename = `Letsedrop_Instagram_Photo_${item.index + 1}.jpg`;
      const disposition = response.headers.get('Content-Disposition');
      if (disposition && disposition.includes('filename=')) {
        const matches = /filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/.exec(disposition);
        if (matches != null && matches[1]) {
      ...
      triggerBrowserDownload(blob, filename, contentType);

      saveToHistory({
        url: urlToSend,
        title: `${mediaData?.title || 'Instagram Photo'} (#${item.index + 1})`,
        thumbnail: item.thumbnail || item.url,
        platform: 'INSTAGRAM',
        filename,
      });

      showToast(`Foto #${item.index + 1} berhasil diunduh.`);
    } catch (err: unknown) {
      const error = err as Error;
      setErrorMessage(error.message || 'Gagal mengunduh foto.');
    } finally {
  ...
  const handleDownloadAllImages = async () => {
    if (!mediaData?.items || mediaData.items.length === 0) return;
    ...
    try {
      for (let i = 0; i < items.length; i++) {
        ...
        setBatchProgressText(`Mengunduh foto ${i + 1} dari ${items.length}...`);
        await downloadSingleImage(currentItem, analyzedUrl);
        if (i < items.length - 1) await new Promise((resolve) => setTimeout(resolve, 800));
      }
      setSuccessInfo({
        filename: `${items.length} Foto Berhasil Diunduh`,
        platform: 'INSTAGRAM CAROUSEL',
      });
      showToast('Semua foto berhasil dikirim ke perangkat Anda.');
    } catch {
      setErrorMessage('Terjadi kendala saat mengunduh beberapa foto.');
    } finally {
  ...
  const handleDownloadSelectedImages = async () => {
    if (!mediaData?.items || selectedItems.length === 0) return;
    setIsBatchDownloading(true);
    setErrorMessage(null);
    const itemsToDownload = mediaData.items.filter((it) => selectedItems.includes(it.index));

    try {
      for (let i = 0; i < itemsToDownload.length; i++) {
        const currentItem = itemsToDownload[i];
        setBatchProgressText(`Mengunduh ${i + 1} dari ${itemsToDownload.length}...`);
        await downloadSingleImage(currentItem, analyzedUrl);
        if (i < itemsToDownload.length - 1) await new Promise((resolve) => setTimeout(resolve, 800));
      }
      setSuccessInfo({
        filename: `${itemsToDownload.length} Foto Terpilih Berhasil Diunduh`,
        platform: 'INSTAGRAM CAROUSEL',
      });
      showToast('Semua foto terpilih berhasil dikirim.');
    } catch {
      setErrorMessage('Terjadi kendala saat mengunduh beberapa foto terpilih.');
    } finally {
      setIsBatchDownloading(false);
      setBatchProgressText('');
    }
  ...
  const toggleSelectAll = () => {
    ...
    if (selectedItems.length === mediaData.items.length) {
      setSelectedItems([]);
    } else {
      setSelectedItems(mediaData.items.map((it) => it.index));
    }
  };
  ...
  const platforms = [
    { name: 'TikTok', icon: '🎵', badge: 'Video & Audio' },
    { name: 'YouTube', icon: '▶️', badge: 'MP4 HD & MP3' },
    { name: 'Instagram', icon: '📸', badge: 'Reels & Carousel' },
    { name: 'Facebook', icon: '📘', badge: 'Video Publik' },
    { name: 'X / Twitter', icon: '🐦', badge: 'Klip Video' },
    { name: 'Reddit', icon: '🤖', badge: 'Video Bersuara' },
  ];
          ...
          <div className="space-y-10">
            <div className="text-center space-y-3 pt-4">
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-neutral-900/90 border border-neutral-800 text-[11px] font-semibold text-neutral-300 mb-2 shadow-inner">
                <Icons.Sparkles />
                <span>Pengunduh Media Publik Tercepat & Bersih</span>
              </div>
              <h1 className="text-4xl sm:text-6xl font-black tracking-tight text-white leading-tight">
                Download. Drop. <span className="bg-gradient-to-r from-indigo-400 via-violet-400 to-fuchsia-400 bg-clip-text text-transparent">Done.</span>
              </h1>
              ...
              <form onSubmit={handleAnalyze} className="space-y-4">
                <div className="relative">
                  <input
                    ...
                    disabled={isAnalyzing || isDownloading || isBatchDownloading}
                    required
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-neutral-400">
                  ...
                  <button
                    type="submit"
                    disabled={isAnalyzing || isDownloading || isBatchDownloading || !urlInput.trim()}
              ...
              {errorMessage && (
                <div className="mt-4 p-4 rounded-2xl bg-rose-950/40 border border-rose-800/60 text-rose-200 text-xs sm:text-sm flex items-start gap-3 animate-fade-in">
                  ...
                  <div className="flex-1 leading-relaxed">
                    <p className="font-bold text-rose-300">Gagal Memproses</p>
            ...
            {mediaData && mediaData.type === 'image' && mediaData.items && (
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-neutral-800 pb-5">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-2.5 py-0.5 bg-neutral-800 rounded-md text-[10px] font-bold uppercase text-neutral-300">
                        {mediaData.items.length > 1 ? `Carousel (${mediaData.items.length} Foto)` : 'Foto Tunggal'}
                      </span>
                    </div>
                  ...
                  {mediaData.items.length > 1 && (
                    <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                      ...
                      >
                        {selectedItems.length === mediaData.items.length ? 'Batal Pilih' : 'Pilih Semua'}
                      </button>
                      <button
                        type="button"
                        onClick={handleDownloadSelectedImages}
                        disabled={isBatchDownloading || selectedItems.length === 0}
                      ...
                      >
                        <span>Unduh Terpilih ({selectedItems.length})</span>
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        {isBatchDownloading ? (
                          <>
                            ...
                            <span>{batchProgressText || 'Mengunduh...'}</span>
                          </>
                        ) : (
                          <>
                            <Icons.Download />
                    ...
                    return (
                      <div
                        ...
                        className={`group relative bg-neutral-950 border rounded-2xl overflow-hidden flex flex-col transition duration-200 ${
                          isSelected ? 'border-indigo-500 ring-2 ring-indigo-500/30' : 'border-neutral-800 hover:border-neutral-700'
                        }`}
                      >
                        {mediaData.items && mediaData.items.length > 1 && (
                          <div
                        ...
                        <div className="p-2.5 border-t border-neutral-800/80 flex items-center justify-between gap-2 bg-neutral-950">
                          ...
                          <button
                            ...
                            disabled={downloadingItemIndex === item.index || isBatchDownloading}
                          ...
                          >
                            {downloadingItemIndex === item.index ? (
              ...
              <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-5 sm:p-7 shadow-2xl space-y-6 animate-fade-in">
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="w-full md:w-72 flex-shrink-0 aspect-video bg-neutral-950 rounded-2xl overflow-hidden relative border border-neutral-800 shadow-md">
                    {mediaData.thumbnail ? (
                      <img src={mediaData.thumbnail} alt={mediaData.title} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1 space-y-4">
                    <div>
                      <h3 className="text-lg sm:text-xl font-extrabold text-white leading-snug">{mediaData.title}</h3>
                      <p className="text-xs text-neutral-400 mt-1">
                        Pengunggah: <span className="text-neutral-200 font-medium">{mediaData.uploader || 'Publik'}</span>
                      </p>
                    </div>

                    <div className="space-y-2">
                      <label className="text-xs font-semibold text-neutral-300 uppercase tracking-wider block">Pilih Format:</label>
                      <div className="grid grid-cols-2 gap-2">
                        {mediaData.formats?.map((fmt) => (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <span className="text-xs px-2 py-0.5 rounded font-mono font-bold bg-neutral-800 text-indigo-300 uppercase">{fmt.extension}</span>
                    ...
                    <div className="pt-2">
                      ...
                      >
                        {isDownloading ? (
                          ...
                          <>
                            <Icons.Download />
                            <span>Download {selectedFormatId === 'audio' ? 'Audio MP3' : 'Video MP4'}</span>
                          </>
                        )}
                      </button>
            ...
            {successInfo && (
              <div className="bg-neutral-900 border border-emerald-900/60 rounded-3xl p-6 shadow-2xl space-y-4 text-center animate-fade-in">
                ...
                <div>
                  <h3 className="text-lg font-bold text-white">Download Selesai Dimulai</h3>
                  <p className="text-xs text-neutral-400 mt-1">File sedang disimpan oleh browser Anda.</p>
                </div>
                <div className="bg-neutral-950 p-3.5 rounded-2xl max-w-md mx-auto text-left text-xs space-y-1 font-mono text-neutral-300 border border-neutral-800">
                  <div className="flex justify-between">
                    ...
                    <span className="truncate max-w-[200px] text-emerald-400">{successInfo.filename}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-neutral-500">Platform:</span>
                    <span>{successInfo.platform}</span>
                  </div>
                </div>
                <button
                  type="button"
            ...
            <div className="pt-4">
              <h2 className="text-center text-xs uppercase tracking-widest text-neutral-500 font-bold mb-5">Platform yang Didukung</h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                {platforms.map((p) => (
                  ...
                  >
                    ...
                    <span className="text-xs font-bold text-neutral-200 mt-1">{p.name}</span>
                    <span className="text-[10px] text-neutral-500">{p.badge}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === 'history' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-white">Riwayat Unduhan</h2>
                <p className="text-xs text-neutral-400 mt-0.5">Disimpan secara lokal di browser perangkat Anda.</p>
              </div>
              {history.length > 0 && (
                <button
                  type="button"
            ...
            {history.length === 0 ? (
              <div className="text-center py-16 bg-neutral-900/30 border border-neutral-800/50 rounded-3xl">
                ...
                <p className="text-sm text-neutral-400">Belum ada riwayat unduhan di Letsedrop.</p>
              </div>
            ) : (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-white truncate hover:text-indigo-400 transition">{item.title}</p>
                        <p className="text-xs text-neutral-400 truncate mt-0.5 font-mono">
                          <span className="uppercase text-indigo-400 text-[10px] bg-neutral-800 px-1.5 py-0.5 rounded mr-1.5 font-sans">{item.platform}</span>
                          {item.filename}
                        </p>
                      </div>
                    </div>
